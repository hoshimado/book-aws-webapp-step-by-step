# §2.4 ローカルサーバーでアプリを起動し、ログイン動作を確認する

## 本節の前提条件

まず、本文の節「§2.2 JWTへ署名する際に使うキーペアを作成」の手順に従って、
キーペアを作成しておいてください。
...
また、本文「§2.3 利用する外部IdP（GoogleのIdP）の設定」に従い、
...
を用意していますので、
こちらをコピーし、リネームしてから環境に応じた値を編集すると便利です。
...
本節は、[こちら](../infrastructure/cloudformation/1-create-s3-for-appzip/README.md)を参照ください。

<br><br><br>

# §2.5.4 CloudFormationの構成内容と適用前準備

本節は、[こちら](../infrastructure/cloudformation/2-backend-infra-vpc-ec2/README.md)を参照ください。
...
「[§2.4](#24-ローカルサーバーでアプリを起動しログイン動作を確認する)」で動作確認済みの  
`.env.development.local` ファイルが、`backend` フォルダー配下に存在していることを確認しておいてください。

また、本文の節「§2.5.6 CloudFrontを作成する」および続く「§2.5.7 動作確認」にて  
「ディストリビューションドメイン名」を取得済みであることを前提とします。


## 本節の操作手順

本文の節「§2.5.8.2 アプリ設定変更とZipの再作成、アップロード」の案内にしたがって、`./backend` フォルダー内に 
`.env.production.only.ec2` ファイルを作成してください。

「[§2.4](#24-ローカルサーバーでアプリを起動しログイン動作を確認する)」で案内した  
...
この行のコメントアウトを解除し、  
「§2.5.7 動作確認」で取得した「ディストリビューションドメイン名」を使用して、  
HTTPSアクセス用のURLを値として記載してください。

`.env.production.only.ec2` ファイルの編集を終えて保存したら、
本文の手順にしたがってアプリケーション一式をZipファイルにまとめます。

ここで使用するスクリプトは、以下のファイルです
（※「[§2.4](#24-ローカルサーバーでアプリを起動しログイン動作を確認する)」で使用したスクリプトとは**異なります**）。

* `backend` フォルダー配下の  
  [s3-build-appzip4ec2idp.bat.txt](./s3-build-appzip4ec2idp.bat.txt)

続く節「§2.5.8.3 EC2等の再構築」の手順では、以下のCloudFormationテンプレートを使用してください。  
このテンプレートは「[§2.5.5](../infrastructure/cloudformation/2-backend-infra-vpc-ec2/README.md)」で使用したものと**同一**です。
...
本文の節「§3.2.2 S3用のCloudFrontを作成して接続」および続く「§3.2.3 動作確認」にて、  
...
本文の節「§3.3 バックエンドの設定を変更して再公開」の案内にしたがって、`./backend` フォルダー内に  
...
`.env.production.s3.ec2` ファイルの編集を終えて保存したら（**リスト3.7**）、  
本文の手順にしたがって、アプリケーション一式をZipファイルにまとめます。
その際に使用するスクリプトは、以下のファイルです。  
（※「[§2.5.8](#258-httpsドメインに合わせてアプリ側を再設定する)」で使用したスクリプトとは異なります）

* `backend` フォルダー配下の  
  [s3-build-appzip4ec2-s3.bat.txt](./s3-build-appzip4ec2-s3.bat.txt)

このzipファイルをS3バケットにアップロードした後の、「続いて、VPCやEC2を再構築します」の手順では、節「2.5.8.3 EC2等の再構築」の手順と同様に、以下のCloudFormationテンプレートを使用してください。  
このテンプレートは「[§2.5.5](../infrastructure/cloudformation/2-backend-infra-vpc-ec2/README.md)」で使用したものと**同一**です。
